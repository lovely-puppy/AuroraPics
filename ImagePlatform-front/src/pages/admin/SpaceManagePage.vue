    ...
    <a-table :columns="columns" :data-source="dataList" :pagination="pagination" @change="handleTableChange">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'spaceLevel'">
          <a-tag style="background-color: #f1f0f0">{{ SPACE_LEVEL_MAP[record.spaceLevel] }}</a-tag>
        </template>
        <!-- 空间类别 -->
        <template v-if="column.dataIndex === 'spaceType'">
          <a-tag style="background-color: #f1f0f0">{{ SPACE_TYPE_MAP[record.spaceType] }}</a-tag>
        </template>
        <!-- 使用情况 -->
        ...
        <template v-else-if="column.key === 'action'">
          <a-space wrap>
            ...
            <a-button type="link" style="background-color: red; color: white" @click="doDelete(record.id)">删除 </a-button>
          </a-space>
        </template>
      </template>
    </a-table>
