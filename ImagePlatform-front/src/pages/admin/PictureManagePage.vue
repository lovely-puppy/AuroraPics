    ...
    <a-table :columns="columns" :data-source="dataList" :pagination="pagination" @change="handleTableChange">
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'action'">
          <a-space wrap>
            <a-button
              v-if="record.reviewStatus !== PIC_REVIEW_STATUS_ENUM.REVIEWED"
              type="link"
              @click="handleReview(record, PIC_REVIEW_STATUS_ENUM.REVIEWED)"
              >通过
            </a-button>
            <a-popconfirm title="确认拒绝吗?" ok-text="确认" cancel-text="取消" @confirm="handleReview(record, PIC_REVIEW_STATUS_ENUM.REJECTED)" @cancel="cancel">
              <a-button v-if="record.reviewStatus !== PIC_REVIEW_STATUS_ENUM.REJECTED" type="link" danger>拒绝</a-button>
            </a-popconfirm>
            <a-button type="link" :href="`/add_picture?id=${record.id}`" target="_blank">编辑</a-button>
            <a-popconfirm title="确认删除吗?" ok-text="确认" cancel-text="取消" @confirm="handleDelete(record.id)" @cancel="cancel">
              <a-button type="link" style="background-color: red;color: white">删除</a-button>
            </a-popconfirm>
          </a-space>
        </template>
      </template>
...
import { deletePictureUsingPost, handlePictureReviewUsingPost, listPictureByPageUsingPost } from '@/api/pictureController.ts'
...
const columns = [
  ...
  { title: '图片审核', dataIndex: 'reviewMessage' , width: 240},
...
const handleReview = async (record: API.Picture, reviewStatus: number) => {
  const reviewMessage = reviewStatus === PIC_REVIEW_STATUS_ENUM.REVIEWED ? '审核员通过' : '审核员拒绝'
  const res = await handlePictureReviewUsingPost({
    id: record.id,
    reviewStatus,
    reviewMessage,
  })
