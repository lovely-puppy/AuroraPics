package com.wws.imageplatformback.manager.sharding;
...
public class DynamicShardingManager {
  ...
  private void updateShardingTableNodes() {
    ...
    String newActualDataNodes =
        tableNames.stream()
            .map(tableName -> "image_platform." + tableName) // 确保前缀合法
    ...
    if (shardingRule.isPresent()) {
      ...
      List<ShardingTableRuleConfiguration> updatedRules =
              ...
              .map(
                  oldTableRule -> {
                    if (LOGIC_TABLE_NAME.equals(oldTableRule.getLogicTable())) {
                      ...
                      newTableRuleConfig.setDatabaseShardingStrategy(
                          oldTableRule.getDatabaseShardingStrategy());
                      newTableRuleConfig.setTableShardingStrategy(
                          oldTableRule.getTableShardingStrategy());
                      newTableRuleConfig.setKeyGenerateStrategy(
                          oldTableRule.getKeyGenerateStrategy());
  ...
  private ContextManager getContextManager() {
    try (ShardingSphereConnection connection =
        dataSource.getConnection().unwrap(ShardingSphereConnection.class)) {
