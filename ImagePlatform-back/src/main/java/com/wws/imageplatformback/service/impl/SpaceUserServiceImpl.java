...
import com.wws.imageplatformback.mapper.SpaceUserMapper;
...
public class SpaceUserServiceImpl extends ServiceImpl<SpaceUserMapper, SpaceUser>
  ...
  public List<SpaceUserVO> getSpaceUserVOList(List<SpaceUser> spaceUserList) {
    ...
    Set<Long> userIdSet =
        spaceUserList.stream().map(SpaceUser::getUserId).collect(Collectors.toSet());
    Set<Long> spaceIdSet =
        spaceUserList.stream().map(SpaceUser::getSpaceId).collect(Collectors.toSet());
    // 2. 批量查询用户和空间
